...
public class Field {
    ...
    public void moveRight(int distance) {
        ...
        this.rob.x = this.rob.x + distance < this.data[0].length?this.rob.x + distance:this.data[0].length - 1;
        this.data[this.rob.y][this.rob.x] = this.robot;
        for (int i = old_x; i < this.rob.x; i++) data[this.rob.y][i] = this.trace_on ? this.trace : this.empty;
    }
    ...
    public void moveLeft(int distance) {
        ...
        int old_x = this.rob.x;
        this.rob.x = (this.rob.x - distance >= 0)?this.rob.x - distance:0;
        this.data[this.rob.y][this.rob.x] = this.robot;
        for (int i = old_x; i > this.rob.x; i--) data[this.rob.y][i] = this.trace_on ? this.trace : this.empty;
    }
    ...
    public void moveUp(int distance) {
        ...
        this.rob.y = (this.rob.y - distance >= 0) ?this.rob.y - distance: 0;
        this.data[this.rob.y][this.rob.x] = this.robot;
        for (int i = old_y; i > this.rob.y; i--) data[i][this.rob.x] = this.trace_on ? this.trace : this.empty;
    }
    ...
    public void moveDown(int distance) {
        ...
        int old_y = this.rob.y;
        this.rob.y =  (this.rob.y + distance < data.length)?this.rob.y + distance:data.length - 1;
        this.data[this.rob.y][this.rob.x] = this.robot;
        for (int i = old_y; i < this.rob.y; i++) data[i][this.rob.x] = this.trace_on ? this.trace : this.empty;
        ...
        try {
            switch (cmd.trim().toLowerCase()) {
                ...
                case "down":
                    ...
                    return true;
                case "trace":
                    this.setTrace_on(this.scanner.next().equals("down") ? true : false);
                    return true;
                case "print":
                    this.printField();
                    return true;
